...
หน้า `admin/members.html` ให้ผู้ดูแลบันทึกข้อมูลภายในเพื่อบริการลูกค้าได้ต่อเนื่อง:

- เพิ่มป้ายกำกับ เช่น ลูกค้าประจำ, VIP หรือร้านอาหาร และกรองรายชื่อตามป้าย
- ค้นหาจากชื่อ เบอร์โทร ป้ายกำกับ และหมายเหตุภายใน
- บันทึกหมายเหตุสูงสุด 2,000 ตัวอักษรและป้ายสูงสุด 10 ป้ายต่อลูกค้า
- ดูประวัติการเปลี่ยนแปลง ผู้แก้ และเวลาแบบ Append-only

ข้อมูลส่วนนี้ไม่ส่งผ่าน LINE LIFF และไม่แสดงให้ลูกค้าเห็น ตารางเปิด RLS และอนุญาตเฉพาะ
บัญชี Admin ที่เข้าสู่ระบบ ส่วน `anon` ไม่มี Table Grant สำหรับข้อมูลลูกค้าและข้อมูลภายใน

## Repeat Purchase Insights

Customer Center ช่วยให้ผู้ดูแลเห็นโอกาสเสนอขายซ้ำจากประวัติการซื้อจริง:

- วิเคราะห์แยกตามสินค้าและ Product Variant จากออเดอร์ที่ชำระหรือดำเนินการสำเร็จ
- รวมการซื้อ Variant เดียวกันหลายออเดอร์ในวันเดียวเป็นหนึ่งวันซื้อ ตามเวลา `Asia/Bangkok`
- แสดงจำนวนครั้ง จำนวนชิ้น วันที่ซื้อล่าสุด และช่วงห่างเฉลี่ยระหว่างวันซื้อ
- คำนวณวันที่แนะนำให้เสนอขายซ้ำเมื่อมีข้อมูลอย่างน้อย 2 วันซื้อ
- มี KPI ตัวกรอง และป้ายเตือนลูกค้าที่ถึงวันที่แนะนำแล้ว

ข้อมูลวิเคราะห์เรียกผ่าน RPC แบบ `SECURITY INVOKER` ที่ตรวจบัญชี Admin และเปิดสิทธิ์เฉพาะ
`authenticated`; ลูกค้า LINE LIFF, `anon` และ `service_role` เรียกใช้ไม่ได้
